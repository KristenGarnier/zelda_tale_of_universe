...
BasicGame.Cave.prototype = {
    ...
    create: function () {
        ...
        this.game.world.setBounds(0, 0, 1504, 2016);
        ...
        this.game.add.sprite(0, 0, 'donjon_img');
        ...
        this.donjon_music = this.game.add.audio('cave');
        this.findKey = this.game.add.audio('key');
        this.donjon_music.play('', 0, 0.3, true, false);

        this.niv_chg = this.game.add.group();
        this.niv_chg.enableBody = true;
        var chng = this.niv_chg.create(744, 1966, 'level');
        //var chng = this.niv_chg.create(280,300, 'level');

        this.keys = this.game.add.group();
        this.keys.enableBody = true;
        if (BasicGame.key == false) {
            var key = this.keys.create(884, 111, 'key');
        }

        this.chests = this.game.add.group();
        this.chests.enableBody = true;
        var chest = this.chests.create(736, 700, 'treasure');
        ...
        this.enemiesTotal = 11;
        this.enemiesAlive = 11;

        this.enemies.push(new enemyStrongVertical(0, this.game, this.player, this.fireballs, 754, 1653));
         this.enemies.push(new enemyStrongVertical(1, this.game, this.player, this.fireballs, 780, 1275));
         this.enemies.push(new enemyStrongVertical(2, this.game, this.player, this.fireballs, 328, 846));
         this.enemies.push(new enemyStrongVertical(3, this.game, this.player, this.fireballs, 1090, 711));
         this.enemies.push(new FinalBoss(4, this.game, this.player, this.fireballs, 731, 347));
         this.enemies.push(new EnemyMonster(5, this.game, this.player, this.fireballs, 782, 1080));
         this.enemies.push(new EnemyMonster(6, this.game, this.player, this.fireballs, 378, 756));
         this.enemies.push(new EnemyMonster(7, this.game, this.player, this.fireballs, 130, 882));
         this.enemies.push(new EnemyMonster(8, this.game, this.player, this.fireballs, 681, 621));
         this.enemies.push(new EnemyMonster(9, this.game, this.player, this.fireballs, 834, 621));
         this.enemies.push(new EnemyMonster(10, this.game, this.player, this.fireballs, 1357, 822));
        ...
        this.player = this.game.add.sprite(744, 1940, 'dude');
        ...
        if (BasicGame.queteCave) {
            swal("Nouvelle quête !", "Pour compléter cette quete vous devez tuer tous les monstres du Donjon.");
            $(".dropdown-menu").append('<li class="quete_cave">Tuer les monstres de la cave <span class="pull-right quete_cave">' + BasicGame.monsterCave + '/' + this.enemiesTotal + '</span></li>');
            BasicGame.queteCave = false;
        }
    ...
    update: function () {
        ...
        this.game.physics.arcade.overlap(this.player, this.keys, this.haveKey, null, this);
        this.game.physics.arcade.overlap(this.player, this.chests, this.testChest, null, this);
        this.game.physics.arcade.overlap(this.player, this.triforce, this.triforce_find, null, this);
    ...
    hitEnemy: function (enemy, arrow) {
        ...
        if (destroyed) {
            ...
            $(".dropdown-menu li span.quete_cave").empty();
            BasicGame.monsterCave++;
            if (BasicGame.monsterCave == 11) {
                swal("Bien joué !", "Vous avez tué tous les monstres de ce Donjon", "success");
                $(".dropdown-menu li.quete_cave").remove();
                BasicGame.level += 1;
                this.levelUp(BasicGame.level);
            } else {
                var killed = BasicGame.monsterCave + '/' + this.enemiesTotal;
                $(".dropdown-menu li.quete_cave span").append(killed);
            }
        }
    },
    ...
    haveKey: function (player, key) {
        key.kill();
        this.findKey.play('', 0, 0.5, false, false);
        BasicGame.key = true;
    },

    testChest: function (player, chest) {
        if (BasicGame.key) {
            chest.kill();
            posx = chest.x;
            posy = chest.y;
            this.triforce_drop = this.triforce.create(posx, posy, 'triforce');
        } else {
            swal({
                title: "Oups, le coffre ne s'ouvre pas !",
                text: "Trouvez la clef pour ouvrir ce coffre",
                timer: "2000"
            });
        }
    ...
    triforce_find: function (player, triforce) {
        triforce.kill();
        BasicGame.triforce = true;
        setTimeout('this.endGame', 3000);
        if (BasicGame.triforce) {
            swal({
                title: "Vous avez sauvé hyrule, vous êtes un véritable héros.",
                text: "Vous pouvez désormais vous promener librement dans le ce monde, merci d'avoir joué au jeu, nous espérons que vous avez aprécié !.",
                type:"success"
            });
            $(".dropdown-menu li.quete_princ").remove();
            BasicGame.level += 1;
            this.levelUp(BasicGame.level);
        }
    }
