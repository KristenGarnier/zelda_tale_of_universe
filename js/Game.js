...
BasicGame.Game.prototype = {
    ...
    create: function () {
        ...
        this.game.world.setBounds(0, 0, 1600, 2400);
        //  A simple background for our game
        this.map = this.game.add.tilemap('map');
        this.map.addTilesetImage('tiles');
        this.layerMap = this.map.createLayer('collide');
        this.layerMap.resizeWorld();
        this.map.setCollision(1);
        this.game.add.sprite(0, 0, 'sky');

        this.BGS2 = this.game.add.audio('BGS2');
        this.BGS1 = this.game.add.audio('BGS1');
        ...
        this.BGS1.play('', 0, 0.3, true, false);

        this.niv_chg = this.game.add.group();
        this.niv_chg.enableBody = true;
        var chng = this.niv_chg.create(158, 1830, 'level');

        this.niv_house = this.game.add.group();
        this.niv_house.enableBody = true;
        var house = this.niv_house.create(610, 1800, 'level');
        ...
        this.enemies.push(new EnemyMonster(1, this.game, this.player, this.fireballs, 1259, 1486));
        ...
        this.player = this.game.add.sprite(680, 1817, 'dude');
        ...
        if (BasicGame.quetePrincipale) {
            swal("Quete principale !", "Sauvez le village en récupérant la triforce dans le coffre du donjon des maîtres.");
            $(".dropdown-menu").append('<li class="quete_princ">Recupérer la triforce dans le donjon des maîtres <span class="pull-right quete_princ">0/1</span></li>');
            BasicGame.quetePrincipale = false;
        }
    ...
    update: function () {
        if (this.player.y < 1678) {
            if (BasicGame.queteDepart) {
                swal("Première quête !", "Apprenez à controler link en tuant 4 monstres à l'extérieur du village.");
                $(".dropdown-menu").append('<li class="quete_depart">Tuer des monstres <span class="pull-right quete_depart">0/4</span></li>');
                BasicGame.queteDepart = false;
            }
        }
        //  Honestly, just about anything could go here. It's YOUR game after all. Eat your heart out!
        ...
        this.game.physics.arcade.collide(this.niv_house, this.layerMap);
        ...
        this.game.physics.arcade.overlap(this.player, this.niv_house, this.redirection_house, null, this);
        ...
        if (this.BGS1_play == 0 && this.player.y < 1678) {
            this.BGS2.stop();
            this.BGS1.play('', 0, 0.3, true, false);
            this.BGS1_play = 1;
            this.BGS2_play = 0;
        } else if (this.BGS2_play == 0 && this.player.y > 1678 && this.player.x < 883) {
            this.BGS1.stop();
            this.BGS2.play('', 0, 0.3, true, false);
            this.BGS2_play = 1;
            this.BGS1_play = 0;
        }
    ...
    hitEnemy: function (enemy, arrow) {
        ...
        if (destroyed) {
            ...
            $(".dropdown-menu li span.quete_depart").empty();

            BasicGame.monsterKill++;
            if (BasicGame.monsterKill == 4) {
                swal("Bien joué !", "Vous avez complété la quete n°1", "success");
                $(".dropdown-menu li.quete_depart").remove();
                BasicGame.level += 1;
                this.levelUp(BasicGame.level);
            } else {
                var killed = BasicGame.monsterKill + "/4";
                $(".dropdown-menu li span.quete_depart").append(killed);
            }
        }
    },
    ...
    redirection_house: function (pointer) {
